{{-- resources/views/show_task.blade.php --}}
<!DOCTYPE html>
...
<head>
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    .pill.success{ background: rgba(22,163,74,.10); border-color: rgba(22,163,74,.25); color: #166534; }
    ...
    .hint{ font-size: 12px; color: var(--muted); font-weight: 700; margin-top: 4px; }
...
<body>
  ...
  <main class="container">
    ...
    <section class="hero">
      <div class="hero-top">
        <div>
          <div class="hero-title">{{ $task->subject }}</div>
        </div>

        <div class="hero-actions">
          <a class="btn btn-outline" href="{{ route('s_index') }}">⬅️ العودة للمهام</a>
        </div>
      </div>
    </section>
    ...
    @php
      // اعتبر أن $rate هو تسليم الطالب/التقييم، وقد يكون null
      $isSubmitted = !empty($rate);
    @endphp

    <!-- Card: الحالة -->
    <section class="card">
      <div class="row">
        <div style="font-weight:900; color:var(--primary); font-size:16px;">الحالة</div>

        @if($isSubmitted)
          <span class="pill success">تم التسليم</span>
        @else
          <span class="pill warn">لم يتم التسليم</span>
        @endif
      </div>

      <div style="margin-top:10px; color:#334155; font-weight:800; font-size:14px;">
        <div style="margin-top:6px;">
          <span style="color:var(--muted); font-weight:900;">التقييم:</span>
          {{ $rate->rate ?? 'لم يتم التصحيح بعد' }}
        </div>

        @if(!empty($rate?->url))
          <div style="margin-top:6px;">
            <span style="color:var(--muted); font-weight:900;">رابط التسليم:</span>
            <a href="{{ $rate->url }}" target="_blank" rel="noopener" style="color:var(--primary-2); font-weight:900;">
              فتح الرابط
            </a>
          </div>
        @endif
        ...
        @if(!empty($rate?->file))
          <div style="margin-top:6px;">
            <span style="color:var(--muted); font-weight:900;">الملف المرفوع:</span>
            <a href="{{ asset('storage/' . $rate->file) }}" target="_blank" rel="noopener" style="color:var(--primary-2); font-weight:900;">
              تحميل الملف
            </a>
          </div>
        @endif

        @if(!empty($rate?->submission))
          <div style="margin-top:10px;">
            <span style="color:var(--muted); font-weight:900;">ملاحظاتك:</span>
            <div class="form-control" style="margin-top:6px; background:#f8fafc; white-space:pre-wrap;">
              {{ $rate->submission }}
            </div>
          </div>
        @endif
    ...
    <!-- Card: التسليم -->
    <section class="card">
      <div style="font-weight:900; color:var(--primary); font-size:16px;">تسليم العمل</div>
      <div class="hint">يمكنك تعديل تسليمك وإعادة الحفظ بنفس الفورم.</div>

      <form action="{{ route('task_store') }}" method="POST" enctype="multipart/form-data" style="margin-top:12px; display:grid; gap:10px;">
        @csrf
        <input type="hidden" name="task_id" value="{{ $task->id }}">

        <div class="field">
          <label for="submission">ملاحظات الطالب</label>
          <textarea id="submission" name="submission" rows="4" class="form-control">{{ old('submission', $rate->submission ?? '') }}</textarea>
        </div>

        <div class="field">
          <label for="url">رابط العمل (اختياري)</label>
          <input id="url" type="url" name="url" class="form-control" value="{{ old('url', $rate->url ?? '') }}" placeholder="https://...">
          <div class="hint">إذا وضعت رابطًا، تأكد أنه يعمل ومتاح للمعلم.</div>
        </div>

        <div class="field">
          <label for="file">رفع ملف (اختياري)</label>
          <input id="file" type="file" name="file" class="form-control">
        </div>

        <div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:4px;">
          <button type="submit" class="btn btn-primary">✅ تسليم العمل</button>
          <a class="btn btn-outline" href="{{ route('s_index') }}">إلغاء</a>
        </div>
      </form>
    </section>
  </main>
