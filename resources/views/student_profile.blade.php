  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    @media (max-width: 640px){
      ...
      /* نخفي جدول الديسكتوب على الجوال لأنه كبير */
...
<body>
  ...
  <aside class="sidebar" id="sidebar" aria-label="القائمة الجانبية">
    ...
    <a class="active" href="{{ route('my.profile') }}">ملفي الشخصي</a>
  ...
  <main class="container">
    <section class="hero">
      <div class="hero-title">معلومات الطالب</div>
    </section>
    ...
    <div class="profile-grid">
      ...
      <section class="card">
        <div style="display:flex; justify-content:space-between; align-items:center; gap:10px; flex-wrap:wrap;">
          <div style="font-weight:900; color:var(--primary);">البيانات</div>
          <span class="pill">ID: {{ $student->user_id }}</span>
        </div>

        <div class="kv" style="margin-top:12px;">
          <div class="k">الاسم</div><div class="v">{{ $user->name }}</div>
          <div class="k">العمر</div><div class="v">{{ $student->age }}</div>
          <div class="k">المسار</div><div class="v">{{ $student->track }}</div>
          <div class="k">الحفظ</div><div class="v">{{ $student->memorization }}</div>
          <div class="k">المستوى</div><div class="v">{{ $student->level }}</div>
          <div class="k">الهدف</div><div class="v">{{ $student->goal }}</div>
          <div class="k">الهاتف</div><div class="v">{{ $student->phone }}</div>
          <div class="k">الجزء</div><div class="v">{{ $student->juz }}</div>
          <div class="k">البريد الإلكتروني</div><div class="v">{{ $user->email }}</div>
        </div>
      </section>

      <!-- Card: ملخص سريع -->
      <aside class="card">
        <div style="font-weight:900; color:var(--primary);">ملخص</div>

        <div style="margin-top:12px; display:grid; gap:10px;">
          <div class="v" style="white-space:normal;">
            <span style="font-weight:900; color:#334155;">المسار:</span>
            {{ $student->track }}
          </div>
          <div class="v" style="white-space:normal;">
            <span style="font-weight:900; color:#334155;">الهدف:</span>
            {{ $student->goal }}
          </div>
          <div class="v" style="white-space:normal;">
            <span style="font-weight:900; color:#334155;">الحفظ:</span>
            {{ $student->memorization }}
          </div>
          <div class="v" style="white-space:normal;">
            <span style="font-weight:900; color:#334155;">المستوى:</span>
            {{ $student->level }}
          </div>
        </div>

        <div style="margin-top:12px; color:var(--muted); font-weight:800; font-size:12px;">
        </div>
      </aside>
    </div>
